...
\vest Одной из отличительных особенностей Scheme является то, что продолжения, функционирование
...
\mainindex{number}Scheme поддерживает множество численных типов данных, включая
...
\mainindex{vector}Векторы, как и списки, являются линейными структурами данных, представляющими
...
Данная форма записи означает, что выражение \textbf{\schtrue{}} вычисляется в \textbf{\schtrue{}}, то
...
Как и во многих других языках, пробельные символы (включая конец строки) являются незначащими,
...
Выражение {\cf\bfseries (f 23)} является \index{procedure call} вызовом процедуры,
...
Обычно вызовы процедур можно отличить от таких \textit{специальных форм}\mainindex{special
  form} с помощью поиска синтаксического ключевого слова в первой позиции формы: если
...
%in the expression
При каждом вычислении выражения Scheme в этом месте существует
\textit{продолжение}\index{continuation}, ожидающее результат выражения. Продолжение
...
немедленно вызывает её с аргументом, который является \textit{аварийной процедурой}\index{escape
  procedure}. Эта аварийная процедура может быть вызвана позже с аргументом, ставшим
...
В следующем примере аварийная процедура, представляющая собой продолжение, которое прибавляет 1 к своему
...
\textit{библиотеками}\index{library}. Каждая библиотека содержит определения и выражения. В неё
...
%on a separate line:
Из следующей библиотеки с именем {\cf\bfseries (hello)} экспортируется определение, которое
...
секцию~\extref{lib:simpleiosection}{Simple I/O}). Экспорт {\cf\bfseries hello-world} является
...
главу~\extref{lib:programlibchapter}{Command-line access and exit values}). Затем она
открывает файл с помощью {\cf\bfseries open-file-input-port} (см. библиотечную секцию
~\extref{lib:portsiosection}), образовывая \textit{порт}, т.е.\ связь с файлом как с источником
