...
Программа Scheme состоит из \textit{программы верхнего уровня\index{top-level program}} вместе с
...
Идентификатор\index{identifier} внутри тела библиотеки или программы верхнего уровня может именовать
...
\textit{видимых привязок}\mainindex{binding}, называется {\em окружением}, действующим в
данной точке.
...
Одни формы используются для создания синтаксических абстракций и привязки ключевых
...
формы называются {\em конструкциями привязки}.\mainindex{binding construct}, Некоторые
...
\defining{регион} кода, внутри которого привязка видима. Регион определяется конкретной
...
идентификатора не существует привязки, он считается \defining{непривязанным}.\mainindex{bound}
...
\mainindex{exceptional situation}В данном стандарте рассматриваются разнообразные исключительные
...
см. библиотечную секцию~\extref{lib:exceptionssection}{Exceptions}.
...
секции~\extref{lib:conditionssection}{Conditions}.
...
\textit{продолжаемыми}\index{continuable exception}. Для большинства исключительных ситуаций,
...
корректной программы в связи с каким-либо \defining{ограничением реализации}. Например, реализация,
...
В данном стандарте фраза ``возбуждено исключение'' используется синонимично с ``должно быть возбуждено
исключение''. В данном стандарте используется фраза ``исключение с типом состояния \var{t}'' для указания,
...
синтаксиса. \defining{Нарушение синтаксиса} -- это ошибка по отношению к синтаксису тел библиотек,
...
\defining{безопасными библиотеками}. Библиотеки и программы верхнего уровня, импортирующие
...
``false'' при обращении к {\bfseries\schfalse{}}. \mainindex{true}\mainindex{false}\vspace{-2mm}
...
\vest Если сообщается, что выражение ``возвращает неопределённые значения'', оно должно
...
значения. \mainindex{unspecified behavior}\mainindex{unspecified values}\vspace{-1.2mm}
...
адресуются к областям памяти\mainindex{location} или последовательностям областей
...
\textbf{\ide{eqv?}} (секция~\ref{equivalencesection}) последнему объекту, сохранённому в области памяти перед
считыванием.
...
Каждая область памяти помечается признаком её использования. Переменная или объект никогда
...
адресующийся к области памяти, связан с флагом, указывающим на изменяемость\index{mutable} или
неизменяемость\index{immutable} объекта. Литеральные константы, строки, возвращаемые
{\bfseries\ide{symbol->string}}, записи без изменяемых полей и другие значения, явно
...
\section{Чистая хвостовая рекурсия}\vspace{-2.2mm}
...
Реализации Scheme должны быть {\em чистыми хвост-рекурсивными}\mainindex{proper tail
  recursion}. Вызовы процедур, находящиеся в конкретных синтаксических контекстах, называемых
...
конструкциях из библиотеки \textbf{\rsixlibrary{base}} описаны в секции
~\ref{basetailcontextsection}.\vspace{-2.2mm}

%\section{Dynamic extent and the dynamic environment}
\section{Динамический экстент и динамическое окружение}\vspace{-2.2mm}
...
Интервал между запуском и возвращением процедуры называется \defining {динамическим экстентом}
...
(библиотечная секция~\extref{lib:exceptionssection}{Exceptions}) -- к текущему
